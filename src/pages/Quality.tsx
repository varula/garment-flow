    ...
    <AppLayout>
      <div className="flex gap-3">
        <div className="flex-1 min-w-0">
          ...
          <DataTable title="QC Inspection Records" headers={["ID", "PO", "Buyer", "Style", "Stage", "Inspected", "Passed", "Failed", "DHU%", "Defects", "Inspector", "Date", "Result"]}>
            {qualityData.map((q, i) => (
              <Tr key={q.id} onClick={() => setSel(q)} selected={sel?.id === q.id} index={i}>
                ...
                <Td className="text-foreground text-[11px]">{q.buyer}</Td>
                ...
                <Td>
                  <div className="flex gap-0.5 flex-wrap">
                    ...
                    {q.defects.length > 2 && <span className="text-muted-foreground text-[10px]">+{q.defects.length - 2}</span>}
                  </div>
                </Td>
          ...
          {/* Pareto */}
          <div className="bg-card border border-border rounded-lg p-3.5 mt-3">
            <div className="text-foreground font-semibold text-[13px] mb-3">Defect Pareto Analysis</div>
            {topDefects.map(([d, cnt]) => (
              <div key={d} className="flex items-center gap-2 mb-2">
                <span className="text-muted-foreground text-[11px] w-40 flex-shrink-0">{d}</span>
                <div className="flex-1 bg-secondary rounded h-[18px] overflow-hidden">
                  <div className="h-full bg-destructive rounded flex items-center pl-1.5" style={{ width: `${(cnt / topDefects[0][1]) * 100}%` }}>
                    <span className="text-destructive-foreground text-[10px] font-semibold">{cnt}</span>
                  </div>
                </div>
                <span className="text-muted-foreground text-[10px] w-8">{cnt}×</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {sel && (
          <DetailPanel title="QC Detail" subtitle={
            <>
              ...
              <div className="mb-2.5"><StatusBadge label={sel.result} /></div>
            </>
          }>
            ...
            {sel.result === "Fail" && (
              <div className="mt-2.5 bg-destructive/5 border border-destructive/30 rounded-md p-2.5">
                <div className="text-destructive text-[11px] font-semibold mb-1">⚠ Action Required</div>
                <div className="text-destructive/80 text-[10px]">Lot failed QC. Schedule rework or return to laundry before proceeding.</div>
              </div>
            )}
            <div className="mt-2.5">
              <div className="text-muted-foreground text-[10px] uppercase mb-1.5">Defect Types</div>
