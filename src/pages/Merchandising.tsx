    ...
    <AppLayout>
          ...
          {productionOrders.map((o, i) => {
            ...
            return (
              <Tr key={o.id} index={i}>
                ...
                <Td>
                  <div className="text-muted-foreground text-[12px]">{o.shipDate}</div>
                  <div className={`text-[11px] font-medium ${daysLeft < 7 ? "text-destructive" : daysLeft < 14 ? "text-warning" : "text-success"}`}>
      ...
      {tab === "samples" && (
        <DataTable headers={["Sample ID", "Style", "Buyer", "Type", "Attempt", "Submitted", "Response", "Status"]}>
          {sampleData.map((s, i) => (
            <Tr key={s.id} index={i}>
              <Td className="text-primary text-[12px]">{s.id}</Td>
              ...
              <Td className="text-foreground text-[12px] text-center">{s.attempt}</Td>
              <Td className="text-muted-foreground text-[12px]">{s.sub}</Td>
              <Td className="text-muted-foreground text-[12px]">{s.res}</Td>
        ...
        <DataTable headers={["Style", "Buyer", "Version", "Spec Sheet", "BOM", "Artwork", "Updated", "Comments", "Status"]}>
          {techPackData.map((tp, i) => (
            <Tr key={tp.style} index={i}>
              <Td className="text-muted-foreground text-[12px]">{tp.style}</Td>
              <Td className="text-foreground text-[12px] font-medium">{tp.buyer}</Td>
              <Td className="text-muted-foreground text-[12px]">{tp.ver}</Td>
              ...
              <Td className="text-muted-foreground text-[12px]">{tp.upd}</Td>
              <Td className="text-muted-foreground text-[12px]">{tp.cmts > 0 ? `💬 ${tp.cmts}` : "—"}</Td>
      ...
      {tab === "shipments" && (
        <DataTable headers={["PO #", "Buyer", "Ex-Factory", "ETD", "ETA", "Container", "Ship Qty", "Balance", "Mode", "Status"]}>
          {shipmentData.map((s, i) => (
            <Tr key={s.id} index={i}>
              <Td className="text-primary font-semibold text-[12px]">{s.po}</Td>
              <Td className="text-foreground text-[12px] font-medium">{s.buyer}</Td>
              ...
              <Td className={`text-[12px] font-semibold ${s.qty - s.packed > 0 ? "text-warning" : "text-success"}`}>{fmt(s.qty - s.packed)}</Td>
